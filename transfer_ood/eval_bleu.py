...
def bleu_eval_pair(generate_file=None,reference_file=None,SIZE=5000):
    ...
    for line in f:
        if(len(hypothesis_list_0)<SIZE):
            hypothesis_list_0.append(line.split())
    f.close()
    f=open(generate_file+'.1','r')
    for line in f:
        if(len(hypothesis_list_1)<SIZE):
            hypothesis_list_1.append(line.split())
    f.close()
    print(len(hypothesis_list_1))
    ...
    for ngram in range(1, 5):
        weight = tuple((1. / ngram for _ in range(ngram)))
        pool = Pool(multiprocessing.cpu_count() - 4)
        bleu = pool.map(run_f, [(reference_list[i], hypothesis_list_0[i], weight) for i in range(SIZE)])
    ...
    for ngram in range(1, 5):
        weight = tuple((1. / ngram for _ in range(ngram)))
        pool = Pool(multiprocessing.cpu_count() - 4)
        bleu = pool.map(run_f, [(reference_list[i], hypothesis_list_1[i], weight) for i in range(SIZE)])
...
if __name__ == '__main__':
    ...
    bleu_eval_pair('outputs/yelp/result_300','/home/mlsnrs/data/hrz/transfer4/data/yelp/sentiment.train')
