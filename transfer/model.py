...
class Autoencoder(object):
    def __init__(self, sess, dataloader, FLAGS, word_embeddings=None):
        ...
        self.class_logit_mean=tf.reshape(self.modules["classifier_layer_mean"](self.hidden_state),[-1])
        self.class_probs=tf.sigmoid(self.class_logit_mean) #batch_size
        ...
        self.class_logit_covari=tf.reshape(self.modules["classifier_layer_covari"](covar),[-1])
        self.class_probs1=tf.sigmoid(self.class_logit_covari)
        ...
        self.classifier_loss_mean = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=self.inputs["src_style"],logits=self.class_logit_mean))
        self.classifier_loss_covari = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=self.inputs["src_style"],logits=self.class_logit_covari))
        self.classifier_loss = 0*self.classifier_loss_mean+0.1*self.classifier_loss_covari
        ...
        self.min_mle = self.optimizer.minimize(self.mle_loss+self.classifier_loss,var_list=self.trainable_weights)

        self.min_covari = self.optimizer.minimize(self.classifier_loss_covari,var_list=self.modules["classifier_layer_covari"].trainable_weights)
      

        tf.summary.scalar("mle_loss",self.mle_loss)
        tf.summary.scalar("classifier_loss",self.classifier_loss)
        tf.summary.scalar("classifier_loss_mean",self.classifier_loss_mean)
        tf.summary.scalar("classifier_loss_covari",self.classifier_loss_covari)
    ...
    def __create_modules__(self):   
        return {
            ...
            "classifier_layer_mean" : tf.layers.Dense(1),
            "classifier_layer_covari" : tf.layers.Dense(1),
    ...
    def save_data(self,dataloader,file_name,logit_neg,logit_pos):
        ...
        for batch in dataloader.batches:
            feed_dict=self.create_feed_dict(batch)
            logits=self.sess.run(self.class_logit_covari,feed_dict=feed_dict)
            for i in range(self.batch_size):
                if batch['src_style'][i]==0 and logits[i]<logit_neg:
               
                    neg_data.append(batch['src_seq'][i])
                  
    
                if batch['src_style'][i]==1 and logits[i]>logit_pos:    
